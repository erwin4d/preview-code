function [prec, prec_sd, recall, recall_sd] = find_prec_recall_other_cumulative(prop_array_ord, prop_array_MLE, k)
  ...
  prec_ord = prop_array_ord(4,:,:,:) ./ (prop_array_ord(4,:,:,:) + prop_array_ord(3,:,:,:));
  prec_sd_ord = 3*std(squeeze(prec_ord(:,:,:,k_idx))');
  prec_ord = mean(squeeze(prec_ord(:,:,:,k_idx))');

  recall_ord = prop_array_ord(4,:,:,:) ./ (prop_array_ord(4,:,:,:) + prop_array_ord(2,:,:,:));
  recall_sd_ord = 3*std(squeeze(recall_ord(:,:,:,k_idx))');
  recall_ord = mean(squeeze(recall_ord(:,:,:,k_idx))');

  prec_MLE = prop_array_MLE(4,:,:,:) ./ (prop_array_MLE(4,:,:,:) + prop_array_MLE(3,:,:,:));
  prec_sd_MLE = 3*std(squeeze(prec_MLE(:,:,:,k_idx))');
  prec_MLE = mean(squeeze(prec_MLE(:,:,:,k_idx))');

  recall_MLE = prop_array_MLE(4,:,:,:) ./ (prop_array_MLE(4,:,:,:) + prop_array_MLE(2,:,:,:));
